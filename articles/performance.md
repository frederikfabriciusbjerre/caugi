...
d-separator** `Z` with
[`caugi::minimal_separator()`](https://caugi.org/reference/minimal_separator.md)—that
...
[`subgraph()`](https://caugi.org/reference/subgraph.md) is fast enough
to beat adjacency-list constructors that rebuild no indexes at all.
...
- Even on [`subgraph()`](https://caugi.org/reference/subgraph.md), where
  the CSR has to be rebuilt, it stays in the low-single-digit
