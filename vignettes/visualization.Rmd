...
The `caugi` package provides several layout algorithms, each optimized for
different use cases.

### Sugiyama (Hierarchical Layout)

The Sugiyama layout [@sugiyama1981] is ideal for directed acyclic graphs (DAGs).
It arranges nodes in layers to emphasize hierarchical structure and causal flow
from top to bottom, minimizing edge crossings.
...
where edges act as springs and nodes repel each other like charged particles. It
produces organic, symmetric layouts with relatively uniform edge lengths.
...
**Best for:** Publication-quality figures, when accurate distance representation
matters
...
such as treatment/outcome or exposure/response relationships. It arranges nodes
in two parallel lines (rows or columns).
...
partition based on incoming edges. Nodes with no incoming edges are placed in
one group, while nodes with incoming edges are placed in the other. You can also
specify the partition explicitly:

```{r}
#| label: bipartite-explicit
#| fig-width: 5
...
**Best for:** Treatment-outcome structures, exposure-response models, bipartite
...
Named lists is the most intuitive format, and what we have already shown. But
you can also use a named numeric vector:
...
The circle layout places nodes evenly along the perimeter of a circle. The first
node is placed at the top and subsequent nodes proceed counter-clockwise. Edge
structure is ignored, so the layout is the same for any graph with the same
number of nodes.
...
**Best for:** Small graphs, cycle visualization, deterministic fallback when
edge-based layouts are not informative
...
The `plot()` function provides extensive customization options for nodes, edges,
and labels.
...
You can customize edge appearance using the `edge_style` parameter. Styles can
be applied globally, by edge type, by source node, or to individual edges.

Apply the same styling to all edges in the graph:

```{r}
#| label: edge-styling-global
...
```{r}
#| label: edge-styling-per-specific-edge
...
The example below combines global, per-type, per-node, and per-edge styling in a
single plot. More specific styles override more general ones according to the
precedence rules above.

```{r}
#| label: edge-styling-combined
...
indicate uncertainty about edge orientation. These edges appear in PAGs (Partial
Ancestral Graphs). You can customize the circle size:
...
By default, tiered layouts are plotted with boxes around each tier and labels
indicating the tier names (if provided).

```{r}
#| label: tiered-boxes-basic

plot(cg_tiered, tiers = tiers)
```

But you can customize the appearance of tier boxes using the `tier_style`
parameter. Here, for instance, we specify different fill colors for each tier
using a vector.

```{r}
#| label: tiered-boxes-vector
...
For more granular control, you can specify styles for individual tiers. Here, we
customize the "exposures" and "outcome" tiers specifically:

```{r}
#| label: tiered-boxes-by-tier
...
Labels for the tiers can also be customized. Here, for example, we change the
font size and color of the tier labels:

```{r}
#| label: tiered-boxes-labels
...
You can also provide custom labels for each tier instead of using the names from
the tiers object

```{r}
#| label: tiered-boxes-custom-labels
#| fig-width: 5
...
To reset the default, you can call `caugi_default_options()`:

```{r}
#| label: reset-global-options
...
You can compute layouts separately and reuse them. First, compute the layout
coordinates:
